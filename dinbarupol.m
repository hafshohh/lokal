...
enable_sapf = false;
% true  → SAPF aktif: USV menghindari obstacle dinamis via gaya repulsif
% false → SAPF nonaktif: USV hanya ikut ILOS, akan menabrak obstacle dinamis
...
%% ===== ILOS GUIDANCE =====
k_integral     = 0.3;
lookahead_m    = 4.0;   % was 1.5 → lookahead lebih jauh sesuai inersia kapal
cte_thresh_m   = 2.0;   % was 1.5
lookahead_k    = 2.0;   % was 1.5
psi_filter_tau = 0.35;  % was 0.67 → respons heading lebih cepat

%% ===== PID HEADING =====
kp_psi     = 3.0;   % was 2.0 → koreksi heading lebih agresif
ki_psi     = 0.05;
kd_psi     = 0.8;   % was 0.5 → damping lebih untuk cegah overshoot
...
sapf.zeta         = 3.0;
sapf.eta          = 8.0;
sapf.R_infl       = 10;  % ≈ camera.maxRange - obs_rad → aktif sejak pertama terdeteksi
sapf.d_safe       = 2.0;   % jarak minimum dari permukaan obstacle [m]
sapf.d_vort       = 7.0;  % was 4.5 → vortex mulai dari 10m, belok lebih awal
sapf.v_min_factor = 0.55;  % was 0.3 → pertahankan kecepatan cukup untuk belok
sapf.v_max        = u_des;

%% ===== CAMERA PARAMETERS =====
camera = struct();
camera.fov       = deg2rad(120);  % was 120 → wide-angle agar obstacle sisi terdeteksi
camera.minRange  = 0.5;           % minimum detection range [m]
camera.maxRange  = 10.0;          % was 8 → lebih jauh beri waktu cukup untuk manuver
...
obs_dyn1.pos    = [15, 10];
obs_dyn1.vel    = [0, 0.3];
obs_dyn1.rad    = obs_size/2 + 1;
obs_dyn1.active = true;

obs_dyn2 = struct();
obs_dyn2.pos    = [37, 26];
obs_dyn2.vel    = [0, -0.3];
obs_dyn2.rad    = obs_size/2 + 1;
...
if enable_extra_obstacle
    extraObs_gt_m = [34.7 16.3 0.25];  % [cx cy radius] dalam meter
else
...
% Gabungkan kedua segmen lalu smooth sekaligus → tangent di waypoint C² kontinyu
path_raw_full = [seg1_raw; seg2_raw(2:end,:)];
path_smooth_m = smooth_path_g2cbs(path_raw_full, n_per_seg, eps_rdp);

[path_smooth_m, clr_info] = enforce_clearance(path_smooth_m, obstacles_static_m, safe_plan_m);
...
for k = 1:N_max
    ...
    %% ==== CEK WAYPOINT 1 ====
    if ~wp1_passed && norm(pos - waypoint_m) < 3.0
        wp1_passed = true;
    ...
    if enable_extra_obstacle && ~extra_known
        ...
        if seenExtra
            ...
            if ~wp1_passed
                % Belum lewat WP1: replan dua segmen (pos→WP, WP→goal)
                ...
                if ~isempty(path1_g) && ~isempty(path2_g)
                    seg1_re = g2m(path1_g);
                    seg2_re = g2m(path2_g);
                    % Pin endpoints per segmen (sama seperti pipeline planning awal)
                    seg1_re(1,:) = pos;        seg1_re(end,:) = waypoint_m;
                    seg2_re(1,:) = waypoint_m; seg2_re(end,:) = goal_m;
                    % LOS shortcutting per segmen — hapus zigzag D* Lite sebelum smoothing
                    seg1_re = shortcut_path(seg1_re, obstacles_all_m, inflate_m, los_step);
                    seg2_re = shortcut_path(seg2_re, obstacles_all_m, inflate_m, los_step);
                    seg1_re(1,:) = pos;        seg1_re(end,:) = waypoint_m;
                    seg2_re(1,:) = waypoint_m; seg2_re(end,:) = goal_m;
                    path_global_m_re = [seg1_re; seg2_re(2:end,:)];
                else
                ...
                if ~isempty(path_g_re)
                    seg_re = g2m(path_g_re);
                    seg_re(1,:) = pos; seg_re(end,:) = goal_m;
                    % LOS shortcutting — hapus zigzag D* Lite sebelum smoothing
                    seg_re = shortcut_path(seg_re, obstacles_all_m, inflate_m, los_step);
                    seg_re(1,:) = pos; seg_re(end,:) = goal_m;
                    path_global_m_re = seg_re;
                else
                    warning('[Replan] Gagal — tetap pakai path lama');
                    replan_done = true;
                    continue;
            ...
            % Smoothing & clearance — pipeline identik dengan planning awal
    ...
    idx_lookahead = min(idx_nearest + 16, size(path_smooth_m, 1));  % was +5 → ~8m ahead
    ...
    if seen1 && enable_sapf
        ...
        if d_obs1 < sapf.R_infl
            % 1 - x^2 → w besar di jarak jauh, makin penuh saat dekat
            w1  = max(0, min(1, 1 - (d_obs1 / sapf.R_infl)^2));
            psi_des    = wrap_angle(psi_ilos + w1 * wrap_angle(psi_sapf1 - psi_ilos));
            % Kecepatan hanya dikurangi setelah masuk zona vortex (d < d_vort)
            if d_obs1 < sapf.d_vort
                u_cmd_base = v_sapf1;
            end
        end
    end
    ...
    if seen2 && enable_sapf
        ...
        if d_obs2 < sapf.R_infl
            w2  = max(0, min(1, 1 - (d_obs2 / sapf.R_infl)^2));
            psi_des    = wrap_angle(psi_des + w2 * wrap_angle(psi_sapf2 - psi_des));
            if d_obs2 < sapf.d_vort
    ...
    %% FORCES
    %% ================================================================
    Fx_ff = -(A2*u_cmd_g + A3*abs(u_cmd_g)*u_cmd_g + A4*(abs(u_cmd_g)^2)*u_cmd_g) / A18;
    Fx    = Ku * (u_cmd_g - nu(1)) + Fx_ff;
...
if enable_sapf
    ...
    sapf_label = 'SAPF OFF (no avoidance)';
end
title(sprintf('Trajektori USV — %s | Camera-Replan Statis — %dx speed', sapf_label, playback_speed));
...
title('Jarak USV dengan Rintangan Dinamis');
...
function [psi_ref, v_ref, F_rep_out] = sapf_local_planner(pos, psi, p_goal, obsDyn, v_ref_in, sapf) %#ok<INUSL>
    ...
    if obsDyn.active
        ...
        if d_s < sapf.R_infl
            ...
            % cross_z = (to_goal × to_obs)_z
            % cross_z > 0 → obstacle di KIRI → putar F_rep CCW (+1) → USV belok KANAN
            % cross_z ≤ 0 → obstacle di KANAN → putar F_rep CW  (-1) → USV belok KIRI
            to_goal = p_goal - pos;
