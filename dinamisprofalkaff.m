...
%
% Perbaikan:
%   - SAPF dengan cone-based repulsion (bukan 360° penuh)
%   - Deteksi kamera dengan FOV, range, dan noise
%   - Speed modulation saat dekat obstacle
%   - Integrasi seamless dengan ILOS guidance
...
%% ===== SAPF PARAMETERS (IMPROVED) =====
sapf = struct();
sapf.zeta         = 2;      % attractive gain
sapf.eta          = 10;      % repulsive gain
sapf.R_infl       = 3;      % radius of influence [m]
sapf.d_safe       = 1.5;      % safety distance [m]
sapf.alpha_th     = deg2rad(90);  % cone half-angle untuk deteksi depan
sapf.v_min_factor = 0.05;      % minimum speed factor saat dekat obstacle
sapf.v_max        = u_des;    % maximum speed [m/s]

%% ===== CAMERA PARAMETERS =====
camera = struct();
camera.fov       = deg2rad(70);  % field of view [rad]
camera.minRange  = 0.5;           % minimum detection range [m]
camera.maxRange  = 10.0;           % maximum detection range [m]
...
obs_dyn1.pos    = [15, 7.5];
obs_dyn1.vel    = [0, 0.3];
obs_dyn1.rad    = obs_size/2 + 0.3;  % effective radius + margin
...
extraObs_gt_m = [39.2 14 0.25];
wp_pass_dist    = arrive_dist + 1.0;
extra_obs_done  = false;
extra_obs_camera_seen = false;          % <-- tambah
obstacles_all_m = obstacles_static_m;

obs_extra = struct();                   % <-- tambah
obs_extra.pos    = extraObs_gt_m(1:2); % <-- tambah
obs_extra.rad    = extraObs_gt_m(3) + 0.3; % <-- tambah
obs_extra.active = false;              % <-- tambah
%% ===== SIMULASI =====
dt    = 0.02;
T_max = 300;
...
%% ===================================================================
%% 4. OBSTACLE TAMBAHAN — muncul setelah waypoint
%% ===================================================================
...
for k = 1:N_max
    ...
    %% ---- Cek obstacle tambahan (replan) ----
    %% ---- Aktifkan obs_extra setelah lewat WP1 ----
if ~extra_obs_done && norm(pos - waypoint_m) < wp_pass_dist
    obs_extra.active = true;
    fprintf('[Info] t=%.1f s | obs_extra aktif, menunggu deteksi kamera\n', t);
    extra_obs_done = true;
end

%% ---- Replan saat kamera deteksi extra obstacle ----
if obs_extra.active && ~extra_obs_camera_seen
    [seen_extra, meas_extra] = camera_detect(pos, psi, obs_extra, camera);

    if seen_extra
        extra_obs_camera_seen = true;
        fprintf('[Replan] t=%.1f s | Kamera deteksi extra obs → mulai replan\n', t);
    ...
    %% ==== SAPF LOCAL OBSTACLE AVOIDANCE (CAMERA-BASED) ====
    % Cari titik path terdekat sebagai local goal untuk SAPF
    dist_to_path = vecnorm(path_smooth_m - pos, 2, 2);
    [~, idx_nearest] = min(dist_to_path);
    
    % Look-ahead point pada path (beberapa titik di depan)
    idx_lookahead = min(idx_nearest + 5, size(path_smooth_m, 1));
    p_goal_local = path_smooth_m(idx_lookahead, :);
    
    % Pilih obstacle terdekat yang terdeteksi kamera
    psi_des = psi_ilos;
    u_cmd_base = u_des;
    
    % Proses obstacle 1 jika terdeteksi
    if seen1
        obs_for_sapf = struct();
        ...
        % Blending berdasarkan jarak
        d_obs1 = meas1.dist - obs_dyn1.rad;
        if d_obs1 < sapf.R_infl
            w1 = 1 - (d_obs1 / sapf.R_infl);
            w1 = max(0, min(1, w1))^2;  % quadratic falloff
            psi_des = wrap_angle(psi_ilos + w1 * wrap_angle(psi_sapf1 - psi_ilos));
            u_cmd_base = v_sapf1;
        end
    end
    
    % Proses obstacle 2 jika terdeteksi
    if seen2
        obs_for_sapf = struct();
    ...
    if d1 < obs_dyn1.rad
        fprintf('COLLISION dengan obstacle 1 pada t=%.2f s (d=%.3f m)\n', t, d1);
    end
    if d2 < obs_dyn2.rad
        fprintf('COLLISION dengan obstacle 2 pada t=%.2f s (d=%.3f m)\n', t, d2);
    end
...
%% --- Figure 1b: Path with Replan ---
figure('Name','Fig 1b: Path dengan Replan','Position',[50 620 900 550]);
clf; hold on; axis equal; grid on; box on;
xlabel('X [m]'); ylabel('Y [m]');
axis([0 mapSize_m(2) 0 mapSize_m(1)]);
title('Path: Original + Replan setelah Deteksi Kamera');
...
r_guard_e = r_e + safeDist_show_m;
fill(cx_e + r_e*cos(th), cy_e + r_e*sin(th), 'm','FaceAlpha',0.3,'EdgeColor','none');
plot(cx_e + r_guard_e*cos(th), cy_e + r_guard_e*sin(th), 'm--','LineWidth',0.8);
text(cx_e+0.3, cy_e+0.3, 'Extra Obs','Color','m','FontSize',8);

% Path original (sebelum replan)
hOrig = plot(path_smooth_init(:,1), path_smooth_init(:,2), 'k-', 'LineWidth', 2.0, 'DisplayName', 'Path Original');

% Path replan (setelah deteksi kamera)
if did_replan && ~isempty(path_smooth_replan)
    hReplan = plot(path_smooth_replan(:,1), path_smooth_replan(:,2), 'b--', 'LineWidth', 2.0, 'DisplayName', 'Path Replan');
else
    hReplan = plot(NaN, NaN, 'b--', 'LineWidth', 2.0, 'DisplayName', 'Path Replan (tidak terjadi)');
end

% Trajektori aktual USV
hActual = plot(x_log, y_log, 'g-', 'LineWidth', 1.2, 'DisplayName', 'Trajektori Aktual');

% Titik replan
if ~isempty(replan_idx)
    plot(x_log(replan_idx), y_log(replan_idx), 'bs', 'MarkerFaceColor','b', ...
        'MarkerSize', 10, 'DisplayName', sprintf('Replan di t=%.1fs', t_log(replan_idx)));
end

% Titik kamera mendeteksi extra obs (gunakan extra_obs_camera_seen)
cam_det_idx = find(LOG_replan, 1);  % sama dengan replan_idx
if ~isempty(cam_det_idx)
    plot(x_log(cam_det_idx), y_log(cam_det_idx), 'c^', 'MarkerFaceColor','c', ...
        'MarkerSize', 10, 'DisplayName', sprintf('Kamera deteksi extra obs'));
end
...
xlabel('X [m]'); ylabel('Y [m]');
axis([0 mapSize_m(2) 0 mapSize_m(1)]);
title(sprintf('Trajektori USV dengan SAPF + Camera Detection (%dx speed)', playback_speed));
...
% Extra obstacle placeholder
cx_e = extraObs_gt_m(1); cy_e = extraObs_gt_m(2); r_e = extraObs_gt_m(3);
h_extra_fill = fill(cx_e + r_e*cos(th), cy_e + r_e*sin(th), 'm','FaceAlpha',0.3,'EdgeColor','none','Visible','off');
...
% Trail
h_trail = plot(NaN, NaN, 'b-', 'LineWidth', 2);
...
patch('XData', baseShip(:,1), 'YData', baseShip(:,2), ...
      'FaceColor',[0 0.7 0],'EdgeColor','k','LineWidth',0.8,'Parent', tKapal);
set(tKapal, 'Matrix', makehgtform('translate',[x_log(1) y_log(1) 0],'zrotate',psi_log(1)));

% Camera FOV visualization
h_fov = patch('XData', NaN, 'YData', NaN, 'FaceColor', [0 0.5 1], ...
              'FaceAlpha', 0.15, 'EdgeColor', 'b', 'LineWidth', 0.5);
...
% Dynamic obstacles
h_dyn1 = rectangle('Position',[obs_dyn1.pos(1)-obs_size/2, obs_dyn1.pos(2)-obs_size/2, obs_size, obs_size], ...
                   'FaceColor','r','EdgeColor','k');
h_dyn2 = rectangle('Position',[obs_dyn2.pos(1)-obs_size/2, obs_dyn2.pos(2)-obs_size/2, obs_size, obs_size], ...
                   'FaceColor','b','EdgeColor','k');

% Detection markers
h_det1 = plot(NaN, NaN, 'ro', 'MarkerSize', 12, 'LineWidth', 2);
h_det2 = plot(NaN, NaN, 'bo', 'MarkerSize', 12, 'LineWidth', 2);
...
for k = 1:frame_step:length(t_log)
    ...
    % Update camera FOV visualization
    ...
    if seen1_log(k)
        set(h_det1, 'XData', x1, 'YData', y1, 'Visible', 'on');
    else
        set(h_det1, 'Visible', 'off');
    end
    if seen2_log(k)
        set(h_det2, 'XData', x2, 'YData', y2, 'Visible', 'on');
    else
        set(h_det2, 'Visible', 'off');
    end
    
    % Show extra obstacle after replan
    ...
    det_str = '';
    if seen1_log(k), det_str = [det_str ' Obs1']; end
    if seen2_log(k), det_str = [det_str ' Obs2']; end
    if isempty(det_str), det_str = 'None'; end
    set(hInfoBox, 'String', sprintf(' t=%.1fs | v=%.2f m/s | ψ=%.1f° | Detected:%s', ...
        t_log(k), speed_total, rad2deg(psi_log(k)), det_str));
...
%% --- Figure 3: Distance to Dynamic Obstacles ---
figure('Name','Fig 3: Jarak ke Obstacle Dinamis','Position',[150 110 900 480]);
hold on; grid on; box on;

plot(t_log, dist_dyn1, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Obstacle 1');
plot(t_log, dist_dyn2, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Obstacle 2');

% Mark when detected
det1_idx = find(seen1_log);
det2_idx = find(seen2_log);
if ~isempty(det1_idx)
    scatter(t_log(det1_idx), dist_dyn1(det1_idx), 10, 'r', 'filled', 'HandleVisibility', 'off');
end
if ~isempty(det2_idx)
    scatter(t_log(det2_idx), dist_dyn2(det2_idx), 10, 'b', 'filled', 'HandleVisibility', 'off');
end
...
title('Distance to Dynamic Obstacles');
legend('Location', 'northeast');

%% --- Figure 4: Speed and Heading ---
...
title('Surge Speed (with SAPF slowdown)');
...
fprintf('Min dist to Obs1  : %.3f m (collision if < %.2f m)\n', min(dist_dyn1), obs_dyn1.rad);
fprintf('Min dist to Obs2  : %.3f m (collision if < %.2f m)\n', min(dist_dyn2), obs_dyn2.rad);
fprintf('Camera det rate 1 : %.1f%%\n', 100*sum(seen1_log)/length(seen1_log));
fprintf('Camera det rate 2 : %.1f%%\n', 100*sum(seen2_log)/length(seen2_log));
...
%% SAPF LOCAL PLANNER (IMPROVED)
function [psi_ref, v_ref] = sapf_local_planner(pos, psi, p_goal, obsDyn, v_ref_in, sapf)
% SAPF_LOCAL_PLANNER  Safe Artificial Potential Field
%
%  - Attractive: ke titik referensi path (p_goal)
%  - Repulsive: dari obstacle dinamis
%    * Jika clearance d_s <= d_safe  : repulsive 360°
%    * Jika d_safe < d_s < R_infl    : repulsive hanya di cone depan
%  - Kecepatan diperlambat jika obstacle masuk zona SAPF

    %% 1. Attractive ke path point (goal lokal)
    vec_g = p_goal - pos;
    d_g   = norm(vec_g) + 1e-9;
    F_att = sapf.zeta * (vec_g / d_g);

    %% 2. Repulsive dari obstacle dinamis (berdasarkan CLEARANCE)
    F_rep = [0 0];
    ...
    if obsDyn.active
        ...
        % clearance ke permukaan obstacle
        d_s     = d_o - obsDyn.rad;
        d_s_eff = max(d_s, 1e-3);

        % bearing obstacle terhadap kapal
        ang_o = atan2(r_vo(2), r_vo(1));
        alpha = atan2(sin(ang_o - psi), cos(ang_o - psi));

        % arah tolak (menjauhi obstacle)
        dir_rep = -r_vo / d_o;

        % 1) Kalau sudah masuk d_safe, SELALU repulsive (360°)
        if d_s < sapf.d_safe
            mag_rep = sapf.eta * (1/d_s_eff - 1/sapf.R_infl) / (d_s_eff^2);
            mag_rep = max(mag_rep, 0);
            F_rep   = mag_rep * dir_rep;

        % 2) d_safe < d_s < R_infl: repulsive hanya kalau dalam cone depan
        elseif (d_s < sapf.R_infl) && (abs(alpha) < sapf.alpha_th)
            mag_rep = sapf.eta * (1/d_s_eff - 1/sapf.R_infl) / (d_s_eff^2);
            mag_rep = max(mag_rep, 0);
            F_rep   = mag_rep * dir_rep;
        end
    ...
    %% 3. Resultan gaya -> heading reference
    F_tot = F_att + F_rep;
    ...
    %% 4. Penyesuaian kecepatan (slow-down di sekitar obstacle)
    v_ref = v_ref_in;
...
function res = dsl_less(k1, k2)
    res = k1(1) < k2(1) - 1e-10 || (abs(k1(1) - k2(1)) < 1e-10 && k1(2) < k2(2) - 1e-10);
end
...
function nb = dsl_succ(s, trav, nR, nC, DX, DY, DC)
    ...
    for i = 1:8
        ...
        if nx>=1 && nx<=nC && ny>=1 && ny<=nR && trav(ny, nx)
            nb(end+1,:) = [sub2ind([nR nC], ny, nx), DC(i)];
        end
    end
end
